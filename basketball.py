...
async def basketball_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['basketball_setup']['message_id'] = message.message_id  # Store the setup message_id
...
async def start_game_against_bot(context, chat_id, user_id, bet, mode, points):
    ...
    context.user_data.pop('basketball_setup', None)  # Clear setup state
...
async def basketball_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Handle setup phase
        if 'basketball_setup' in context.user_data:
            setup = context.user_data['basketball_setup']
            ...
            if data == "basketball_mode_guide":
                ...
                context.user_data.pop('basketball_setup', None)
        ...
        # Handle in-game phase
            ...
            if not game or query.message.message_id != game['message_id']:
            ...
            if game['roll_count'][player_key] < game['rolls_needed']:
                ...
                reply_markup = InlineKeyboardMarkup(keyboard)
                message = await send_with_retry(context.bot, chat_id, f"Round {game['round_number']}: Take another shot!", reply_markup=reply_markup)
                ...
                if game['roll_count']['player1'] == game['rolls_needed'] and game['roll_count']['player2'] == game['rolls_needed']:
                    ...
                    keyboard = [[InlineKeyboardButton(f"🏀 Take a Shot (Round {game['round_number']})", callback_data=f"basketball_take_shot_{game['round_number']}")]]
                    reply_markup = InlineKeyboardMarkup(keyboard)
                    message = await send_with_retry(context.bot, chat_id, f"Round {game['round_number']}: @{other_username}, your turn!", reply_markup=reply_markup)
                    game['message_id'] = message.message_id  # Update message_id
        ...
        # Handle post-game and challenge actions
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            message = await send_with_retry(context.bot, chat_id, text, reply_markup=reply_markup)
            game_state['message_id'] = message.message_id  # Store the initial game message_id
        ...
        logger.error(f"Error in basketball_button_handler: {e}")
...
async def basketball_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
    if context.user_data.get('expecting_username') and context.user_data.get('basketball_setup', {}).get('initiator') == user_id:
